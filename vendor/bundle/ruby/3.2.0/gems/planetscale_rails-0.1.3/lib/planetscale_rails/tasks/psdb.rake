...
namespace :psdb do
  task check_ci: :environment do
    use_service_token = ENV["PLANETSCALE_SERVICE_TOKEN"] && ENV["PLANETSCALE_SERVICE_TOKEN_ID"]
    if ENV["CI"]
      raise "For CI, you can only authenticate using service tokens." unless use_service_token

      service_token_config = "--service-token #{ENV["PLANETSCALE_SERVICE_TOKEN"]} --service-token-id #{ENV["PLANETSCALE_SERVICE_TOKEN_ID"]}"
